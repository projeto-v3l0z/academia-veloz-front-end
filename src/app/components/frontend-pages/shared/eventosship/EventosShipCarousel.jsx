...
const EventosShipCarousel = ( {premiados} ) => {
    ...
    useEffect(() => {
        const fetchWinners = async () => {
            ...
            const formattedWinners = premiados.map((winner) => ({
                name: winner.nome, // Assegure-se de que 'name' existe
                institution: winner.emblemas[0]?.instituicao.nome || 'Unama', // Assegure-se de que 'institution' existe
                award: winner.emblemas[0]?.nome || 'Premiação Padrão', // Define um valor padrão se 'award' não existir
                image: winner.imagem// Um valor padrão para a imagem se não estiver disponível
            }));
...
export default EventosShipCarousel;
