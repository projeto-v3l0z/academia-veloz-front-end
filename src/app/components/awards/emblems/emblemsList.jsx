...
const EmblemsList = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError('Erro ao carregar Emblemas');
      } finally {
        setLoading(false);
      }
    };
    ...
    columnHelper.accessor('descricao', {
      ...
      cell: (info) => (
        <Typography
          ...
          color="textPrimary"
          fontWeight={600}
          sx={{ whiteSpace: 'nowrap', maxWidth: '400px', overflow: 'hidden', textOverflow: 'ellipsis' }}
        >
          {info.getValue()}
        </Typography>
      ),
  ...
  const handleCreateClick = () => {
    router.push('/awards/emblema/create');  // Rota de criação de emblema
  };
  ...
  const handleDeleteClick = async (id) => {
    ...
    try {
      ...
      setError('Erro ao excluir o emblema');
    }
  };

  return (
    <ParentCard title="Emblemas">
